...
class CVE:
    ...
    def add_data(self, namespace, data):
        ...
        if namespace == 'cve.org':
            if 'cdb' not in self.json:
                self.json['cdb'] = {}

            # If metadata doesn't exist, prepopulate with sane defaults for a CVE
            if 'metadata' not in self.json['cdb']:
                self.json['cdb']['metadata'] = {
                    "type": "vulnerability",
                    "exploitCode": "unknown",
                    "remediation": "unknown",
            ...
            if 'osvSchema' not in self.json['cdb']:
                self.json['cdb']['osvSchema'] = {}

            # Add the CDB ID if it doesn't exist
            if 'id' not in self.json['cdb']['osvSchema']:
                self.json['cdb']['osvSchema']['id'] = self.cdb_id
            # Add aliases if it doesn't exist, prepopulated with CVE ID
            if 'aliases' not in self.json['cdb']['osvSchema']:
                self.json['cdb']['osvSchema']['aliases'] = [self.id]
            # If aliases exists, but CVE is not in array, append it
            if self.id not in self.json['cdb']['osvSchema']['aliases']:
                self.json['cdb']['osvSchema']['aliases'].append(self.id)
            # FIXME: Always update modified when any values are changed, not just if it's missing
            if 'modified' not in self.json['cdb']['osvSchema']:
                self.json['cdb']['osvSchema']['modified'] = datetime.datetime.utcnow().isoformat() + "Z"
            if 'schema_version' not in self.json['cdb']['osvSchema']:
                self.json['cdb']['osvSchema']['schema_version'] = "1.4.0"

            # Check for a description
            if 'details' not in self.json['cdb']['osvSchema']:
                if '** RESERVED **' not in self.json['namespaces']['cve.org']['description']['description_data'][0]['value']:
                    self.json['cdb']['osvSchema']['details'] = self.json['namespaces']['cve.org']['description']['description_data'][0]['value']
