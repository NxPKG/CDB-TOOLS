...
# Run from /cdb-database/ root directory

# consumes a list of CVEs

# CVE-YEAR-INTEGER
# YEAR/INTxxx/CDB-YEAR-INTEGER.json

input=$1

while IFS= read -r line
do
    CDBID=`echo $line | sed 's/CVE/CDB/'`
    ...
    if [ -f "$FILE" ]; then
      ...
      FOUND="true"
      # If no CDB.references[] array exiosts you'll get an error
      FOUND=`jq -e '.CDB.references|any(. == "'$URL'")' $FILE`
      if [ $FOUND == "true" ]; then
        echo -n "."
      else
        ...
        jq --arg new "$URL" '.CDB.references? += [$new]' $FILE > $TEMP
        # print fix, assumes repo location
        ../cdb-tools/local-scripts/print-json.py $TEMP
